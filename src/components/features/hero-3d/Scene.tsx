...
const WindParticles: React.FC<WindParticlesProps> = ({
  ...
  // Seed-based pseudo-random for purity (deterministic per count/range)
  const particlesPosition = useMemo(() => {
    const positions = new Float32Array(count * 3);
    let seed = count * 13 + rangeX * 7 + rangeY * 11 + rangeZ * 3;
    const pseudoRandom = () => {
      seed = (seed * 16807 + 0) % 2147483647;
      return (seed - 1) / 2147483646;
    };
    for (let i = 0; i < count; i++) {
      positions[i * 3] = (pseudoRandom() - 0.5) * rangeX;
      positions[i * 3 + 1] = (pseudoRandom() - 0.5) * rangeY;
      positions[i * 3 + 2] = (pseudoRandom() - 0.5) * rangeZ;
    }
    return positions;
