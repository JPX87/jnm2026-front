...
import Timeline, { useStepRef } from "@/components/ui/Timeline/Timeline";
...
function Itin1Step0({ preInscStatus }: { preInscStatus: ReturnType<typeof getStatus> }) {
    const ref = useStepRef(0);
    return (
        <div className="relative" ref={ref}>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3 sm:gap-6">
                    <p className="text-lg sm:text-xl md:text-2xl font-bold uppercase">Places limitées</p>
...
function Itin1Step1({ defInscStatus }: { defInscStatus: ReturnType<typeof getStatus> }) {
    const ref = useStepRef(1);
    return (
        <div className="relative" ref={ref}>
            <span className="inline-block w-max bg-(--color-primary) text-(--color-secondary) dark:text-(--color-seconde-black) text-base sm:text-lg font-bold px-5 py-2 rounded-full mb-4">
                À partir du 30 mars
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3 border-2 border-(--color-secondary)/40 dark:border-(--color-seconde-black)/40 rounded-xl px-5 py-4">
                    <span className="text-2xl">✅</span>
                    <p className="text-base sm:text-lg font-bold uppercase">Pré-inscription validée</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 sm:gap-6">
                    <p className="text-lg sm:text-xl md:text-2xl font-bold uppercase">Vous recevrez</p>
                    <span className="bg-(--color-primary) text-(--color-secondary) dark:text-(--color-seconde-black) text-sm sm:text-base font-bold px-4 py-2 rounded-lg uppercase">
...
function Itin2Step0({ galaStatus }: { galaStatus: ReturnType<typeof getStatus> }) {
    const ref = useStepRef(0);
    return (
        <div className="relative" ref={ref}>
            <span className="inline-block w-max bg-(--color-primary) text-(--color-secondary) dark:text-(--color-seconde-black) text-base sm:text-lg font-bold px-5 py-2 rounded-full mb-4">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3 border-2 border-(--color-secondary)/40 dark:border-(--color-seconde-black)/40 rounded-xl px-5 py-4">
                    <span className="text-2xl">🎀</span>
                    <p className="text-base sm:text-lg font-bold uppercase">Rendez-vous le 29 mai</p>
                </div>
            </div>
        </div>
    );
...
export default function InscriptionContent() {
    ...
    const itin2Steps = [{ start: galaStart, end: galaEnd }];
    ...
    return (
        <PageTemplate title="INSCRIPTIONS JNM 2026" className="inscription">
            ...
            <Section title="Itinéraire 1" variant="tertiary" maxWidth="lg"
                titleClassName="!text-left" contentClassName="gap-2">
                <p className="text-2xl sm:text-3xl md:text-4xl font-semibold -mt-4 mb-6">
                    Pack JNM 4 jours ✈️
                </p>
                <Timeline steps={itin1Steps}>
                    <Itin1Step0 preInscStatus={preInscStatus} />
                    <Itin1Step1 defInscStatus={defInscStatus} />
                </Timeline>
            </Section>
            ...
            <Section title="Itinéraire 2" variant="tertiary" maxWidth="lg"
                ...
                <Timeline steps={itin2Steps}>
                    <Itin2Step0 galaStatus={galaStatus} />
                </Timeline>
            </Section>
